	...
	render({ state, device }) {
		return [
			...
			Bird({
				...
				x: birdX,
				y: state.birdY,
				rotation: Math.max(-30, state.birdGravity),
			}),
...
export function newPipe(device) {
...
function didHitPipe(birdY, size, pipes) {
	...
	for (const pipe of pipes) {
		...
		if (
			birdPoints.some(
				(point) =>
					pointInRect(point, topPipeRect) ||
					pointInRect(point, bottomPipeRect)
			)
		) {
			// Bird hit a pipe!
			return true;
